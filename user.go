...
var (
	// user name only consist of a-z, 0-9, or _ sign
	// ^ start
	// $ end
	// + one or more
	// similiar to JS, usernamePattern is the name of the function regxp.MustComplete......
...
func checkUser(username, password string) bool {
	...
	// basically the loop only loop for once
	// the query returns a slice(array) consisting only one element
	var tyu User
	for _, item := range queryResult.Each(reflect.TypeOf(tyu)) {
		// cast itme to user type
		u := item.(User)
...
func signupHandler(w http.ResponseWriter, r *http.Request) {
	...
	if u.Username != "" && u.Password != "" && usernamePattern(u.Username) {
		if addUser(u) {
			...
			//w.Write([]byte("Failed to add a new user"), http.StatusInternalServerError)
		}
	} else {
		fmt.Println("Empty password or username.")
		http.Error(w, "Empty password or username", http.StatusInternalServerError)
		//w.Write([]byte("Empty password or username"), http.StatusInternalServerError)
	}
...
func loginHandler(w http.ResponseWriter, r *http.Request) {
	...
	if checkUser(u.Username, u.Password) {
		// generate token
		// claims is called PAYLOAD, check https://jwt.io purple(2nd) part
